...
func (r *Resolver) resolveDeployment(ctx context.Context, deviceName string) (*deviceDeploymentInfo, error) {
	...
	for i := range deployments.Items {
		...
		if matchesDevice(dep, deviceName) {
			...
			// Look up Service by matching selector against the Deployment's
			// pod template labels, because Service and Deployment names may
			// differ (e.g. deployment "deviceshifu-X-deployment", service "deviceshifu-X").
			svcName, err := r.findServiceForDeployment(ctx, dep)
			if err == nil && svcName != "" {
				info.ServiceName = svcName + "." + deviceShifuNamespace + ".svc.cluster.local"
			}
			return info, nil
		}
	}
...
// findServiceForDeployment finds the Service that selects the given Deployment's pods.
func (r *Resolver) findServiceForDeployment(ctx context.Context, dep *appsv1.Deployment) (string, error) {
	services, err := r.clientset.CoreV1().Services(deviceShifuNamespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return "", fmt.Errorf("listing services in %s: %w", deviceShifuNamespace, err)
	}
...
// selectorMatchesLabels returns true if every key-value pair in selector
// exists in labels.
