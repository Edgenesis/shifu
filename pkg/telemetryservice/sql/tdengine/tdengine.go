...
func SendToTDengine(ctx context.Context, rawData []byte, sqlcs *v1alpha1.SQLConnectionSetting) error {
	...
	if err != nil {
		zlog.Errorf("Error to Connect to tdengine, error %v", err.Error())
		return err
	}

	err = db.insertDataToDB(ctx, rawData)
	if err != nil {
		zlog.Errorf("Error to Insert rawData to DB, errror: %v", err.Error())
		return err
	}
...
func (db *DBHelper) connectToTDengine(ctx context.Context) error {
	...
	zlog.Infof("Try connect to tdengine %v", *db.Settings.DBName)
	return err
}
...
func (db *DBHelper) insertDataToDB(ctx context.Context, rawData []byte) error {
	...
	if err != nil {
		zlog.Errorf("Error to Insert RawData to db, error: %v", err)
		return err
	}

	id, err := result.RowsAffected()
	if err != nil {
		zlog.Errorf("Error to get RowsAffected, error: %v", err)
		return err
	} else if id <= 0 {
		zlog.Errorf("Data insert failed, for RowsAffected is equal or lower than 0")
		return errors.New("insert Failed")
	}

	zlog.Infof("Successfully Insert Data %v to DB %v", string(rawData), db.Settings.DBName)
	return nil
}
