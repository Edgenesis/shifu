...
func New(deviceShifuMetadata *deviceshifubase.DeviceShifuMetaData) (*DeviceShifu, error) {
	...
	if deviceShifuMetadata.KubeConfigPath != deviceshifubase.DeviceKubeconfigDoNotLoadStr {
		...
		if connectionResult.GetErr() != nil {
			zlog.Errorf("Error connecting to PLC: %s", connectionResult.GetErr().Error())
			return nil, err
		}
		...
		if !connection.IsConnected() {
			zlog.Errorf("Cannot Connected to %v", *base.EdgeDevice.Spec.Address)
...
func (ds *DeviceShifu) writeCommandHandlerPlc4x() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			zlog.Errorf("Error when parse request url, error: %v", err)
		...
		if err != nil {
			zlog.Errorf("Error when build request by params, error: %v", err)
		...
		if err := resultData.GetErr(); err != nil {
			zlog.Errorf("Got Error on execute request, error: %v", err)
...
func (ds *DeviceShifu) readCommandHandlerPlc4x() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			zlog.Errorf("Error when marshal result to []byte, error: ", err)
		...
		if err != nil {
			zlog.Errorf("Error when write data into response, error: %v", err)
