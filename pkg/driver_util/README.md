...
### Usage
We have written a sample Dockerfile [`driver_util/examples/simple-alpine/Dockerfile.sample`](/examples/driver_utilutil/simple-alpine/Dockerfile.sample) which demonstrates how you can add the stub into an existing Alpine Docker image

The packaged Docker image takes the following environmental variables, so we need to configure them in the [yaml file](/examples/driver_utilutil/simple-alpine/driver.yaml):
