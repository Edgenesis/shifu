...
func httpCmdlinePostHandler(resp http.ResponseWriter, req *http.Request) {
	...
	timeoutValues, ok := req.URL.Query()["cmdTimeout"]

	if !ok || len(timeoutValues[0]) < 1 {
		log.Printf("Url Param 'cmdTimeout' is missing, setting default to: %v", cmdExecTimeoutSecond)
	} else {
		timeoutSeconds, err = strconv.Atoi(timeoutValues[0])
		log.Printf("Setting timeout to: %v", timeoutSeconds)
		if err != nil {
			log.Printf("cannot convert timeout param: %v to integer", timeoutValues[0])
			resp.WriteHeader(http.StatusBadRequest)
			return
		}
	}
